...
function roundDecimalString(value: string, places = 2): string {
  const negative = value.trim().startsWith("-");
  const abs = value.trim().replace("-", "");
  const [intPart, fracPart = ""] = abs.split(".");
  const paddedFrac = fracPart.padEnd(places + 1, "0");
  const keep = paddedFrac.slice(0, places);
  const roundDigit = paddedFrac.charAt(places);

  let digits = (intPart + keep).replace(/^0+(?=\d)/, "");
  if (roundDigit >= "5") {
    digits = (BigInt(digits) + 1n)
      .toString()
      .padStart(intPart.length + keep.length, "0");
  }

  const intLen = digits.length - places;
  const roundedInt = digits.slice(0, intLen) || "0";
  const roundedFrac = digits.slice(intLen);
  const result = `${roundedInt}.${roundedFrac}`;
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({
  ...
  const displayTotal = roundDecimalString(data.total_revenue);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
      ...
      <div className="p-6">
        ...
        <div className="mt-4 h-6">
          {Number(data.total_revenue) !== Number(displayTotal) && showRaw && (
